...
# =============================================================================
# SỬA 1: ImprovedStructureRecognizer - GIẢM MAX LEVEL = 3
# =============================================================================
class ImprovedStructureRecognizer:
    ...
    def __init__(self) -> None:
        ...
        self.max_heading_level = 3  # ← THÊM: MAX 3 LEVELS
    ...
    def recognize_heading(self, text: str, line: FormattedLine) -> HeadingCandidate:
        ...
        for pattern in numbering_patterns:
            ...
            if match:
                ...
                level = min(self._parse_numbering_level(numbering), self.max_heading_level)  # ← SỬA: MAX 3
    ...
    def _guess_unumbered_level(self) -> int:
        """Đoán level cho heading không đánh số"""
        if self.current_level > 0:
            return min(self.current_level, self.max_heading_level)  # ← SỬA: MAX 3
        else:
            return 1

    # =============================================================================
    # SỬA 4: recognize_bullet - NESTED LIST ĐÚNG
    # =============================================================================
    def recognize_bullet(self, text: str) -> Tuple[Optional[int], str]:
        ...
        for marker in BULLET_MARKERS:
            if stripped.startswith(marker):
                ...
                self._in_list = True
                indent_level = leading_spaces // 4  # ← SỬA: Nested bullets
                return indent_level, clean

        # Indentation-based nesting
        if leading_spaces >= 4 and self._in_list:
            indent_level = (leading_spaces // 4) + 1  # ← SỬA: Nested đúng
...
def _merge_paragraph_fragments(fragments: Sequence[str]) -> str:
    ...
    for fragment in fragments:
        ...
        if not merged:
            ...
            continue
        if merged.endswith(('-', '‐', '‑', '–')):
            merged = merged[:-1] + frag.lstrip()
        elif merged.endswith(('(', '“', '‘', '/', '[', '{')) or (frag[:1] and frag[:1] in ',.;:!?)]}”’'):
            merged = merged + frag
        else:
            merged = merged + ' ' + frag
    return merged.strip()
...
# =============================================================================
# SỬA 2: _merge_spans - BẢO TOÀN 100% BOLD TEXT
# =============================================================================
def _merge_spans(spans: Iterable[FormattedSpan]) -> Tuple[str, str]:
    ...
    for span in spans:
        ...
        if not text.strip():
            ...
            continue
        # ← SỬA: BOLD LUÔN ĐƯỢC ** ** DÙ CÓ ITALIC HAY KHÔNG
        if span.bold:
            formatted = f"**{text}**"
        elif span.italic:
...
# =============================================================================
# SỬA 3: AccurateMarkdownBuilder - TABLE + SPACING CHÍNH XÁC
# =============================================================================
class AccurateMarkdownBuilder:
    """Markdown builder được cải tiến để tạo output chính xác"""
    ...
    def _flush_image_group(self) -> None:
        if self.current_image_group:
            ...
            self.lines.append("")  # ← GIỮ 1 empty line
    ...
    def add_heading(self, level: int, text: str) -> None:
        ...
        hashes = "#" * display_level
        clean_text = text.strip()
        if clean_text:
            self.lines.append(f"{hashes} {clean_text}")
            self.lines.append("")  # ← 1 empty line

    def add_paragraph(self, text: str) -> None:
        self._flush_image_group()
        clean_text = text.strip()
        if clean_text:
            self.lines.append(clean_text)  # ← KHÔNG double empty line
            self.lines.append("")  # ← 1 empty line
    ...
    def add_bullet(self, indent: int, text: str) -> None:
        ...
        prefix = "  " * indent + "* "  # ← Nested đúng
        bullet_text = f"{prefix}{clean_text}"
        self.lines.append(bullet_text)
        self.lines.append("")  # ← 1 empty line

    # ← SỬA: Format table CHÍNH XÁC như target
    def add_table(self, rows: List[List[str]]) -> None:
        ...
        for row in rows[1:]:
            ...
            data_row = "|" + "|".join(f"{str(cell):<{w}}" for cell, w in zip(padded_row, col_widths)) + "|"
            table_lines.append(data_row)
        
        self.lines.extend(table_lines)
        self.lines.append("")  # ← 1 empty line after table
    ...
    # ← SỬA: Clean spacing - MAX 1 empty line
    def build(self) -> str:
        self._flush_image_group()
        cleaned = []
        prev_empty = False
        for line in self.lines:
            if line.strip():
                cleaned.append(line)
                prev_empty = False
            elif not prev_empty:  # ← Chỉ giữ 1 empty line
                cleaned.append("")
                prev_empty = True
...
class AccuratePDFExtractor:
    ...
    def extract(self) -> str:
        ...
        builder = AccurateMarkdownBuilder(title=self.pdf_path.stem)
